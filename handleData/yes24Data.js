...
async function getQueryFromClientToYes24(obj) {

    let dataQuery = await obj;
    console.log("yes24키워드", dataQuery);
    let keyword = await toUnicode.charToUnicode(dataQuery);
    let ccc = testApi(keyword);
    return ccc;
}
...
const testApi = async (keyword) => {

    console.log("testAPi키워드", keyword);
    ...
    fs.writeFile(`Docs/yes24Data.json`,
        JSON.stringify(usedBooks, null, 2), 'utf-8',
        err =>
            err ? console.error('yes24파일 생성에 실패했습니다', err)
                : console.log('yes24 파일 생성에 성공했습니다!')
    );
...
// await fs.writeFile(`Docs/yes24Data.json`,
//     JSON.stringify(usedBooks, null, 2), 'utf-8',
//     (err) => {
//         if (err) throw err;
//         console.log('yes24 파일 생성에 성공했습니다!')
//     }
// );
// fs.writeFile('./meal.json', JSON.stringify(usedBooks), (err) => {
//     if (err) throw err;
//     console.log('OK');
// });

// 파일 생성된 내용 보여주는 콘솔
// const writefileContents = JSON.parse(JSON.stringify(usedBooks));
